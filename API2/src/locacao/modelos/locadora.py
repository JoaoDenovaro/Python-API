...
class Locadora(ModeloBase):
    ...
    nome: orm.Mapped[str] = orm.mapped_column(
        sa.String(100))
    horario_abertura: orm.Mapped[datetime.time
        ] = orm.mapped_column(sa.Time)
    horario_fechamento: orm.Mapped[datetime.time
        ] = orm.mapped_column(sa.Time)
    endereco: orm.Mapped[str] = orm.mapped_column(
        sa.String(255))
